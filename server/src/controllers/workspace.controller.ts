...
const createWorkspace = async (req: Request<{}, {}, CreateWorkspaceDTO>, res: Response, next: NextFunction) => {
  try {
    const { name } = req.body;
...
const editWorkspace = async (
  ...
  try {
    ...
    const data = await workspaceServices.updateOne(_id, req.userId as string, updateQuery);
...
const deleteWorkspace = async (req: Request<{ id: string }, {}, {}>, res: Response, next: NextFunction) => {
  try {
...
const leaveWorkspace = async (req: Request<{ id: string }, {}, {}>, res: Response, next: NextFunction) => {
  try {
    const workspace_id = req.params.id;

    await workspaceServices.leave(req.userId as string, workspace_id);

    res.status(200).json({ message: 'You have successfully left this organization' });
  } catch (error) {
    return next(error);
  }
};

const getMyWorkspaces = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const data = await workspaceServices.findUserWorkspaces(<string>req.userId);
...
const getOneWorkspace = async (req: Request<{ id: string }, {}, {}>, res: Response, next: NextFunction) => {
  try {
    const workspace_id = req.params.id;
...
const inviteUserToWorkspace = async (
  ...
  try {
    ...
    await workspaceServices.inviteUser(req.body.email as string, workspace_id, req.userId as string);
...
const joinWorkspace = async (req: Request<{}, {}, { invite_id: string }>, res: Response, next: NextFunction) => {
  try {
    const invite_id = req.body.invite_id;

    await workspaceServices.joinWithInvite(invite_id);

    res.status(200).json({ message: 'You have successfully joined this workspace' });
  } catch (error) {
    return next(error);
  }
};

const getWorkspaceByInviteId = async (req: Request<{ id: string }>, res: Response, next: NextFunction) => {
  try {
    const invite_id = req.params.id;

    const data = await workspaceServices.findByInviteId(invite_id);
