...
export const resendVerificationEmail: any = createAsyncThunk(
  ...
  async (email: string, thunkApi) => {
    ...
    try {
      const response = await httpInstance.post('/auth/verify-account/resend-email', {
        email,
      });
...
export const verifyAccount: any = createAsyncThunk('authSlice/verifyAccount', async (token: string, thunkApi) => {
  try {
